...
export async function POST(request: Request) {
  ...
  // Prefix with [SALON] so the webhook can identify admin-initiated cancellations
  const markedReason = `[SALON] ${reason}`
  ...
  const res = await fetch(`${CAL_API_BASE}/bookings/${uid}/cancel`, {
    ...
    body: JSON.stringify({ cancellationReason: markedReason }),
  })
